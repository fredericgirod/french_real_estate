...
lon = df_data['longitude'].astype(float)
lat = df_data['latitude'].astype(float)
df_map_data = pd.DataFrame({'lat': lat, 'lon': lon}).astype(float)
st.map(df_map_data.astype(float), zoom=11, use_container_width=True, )
st.write('')
